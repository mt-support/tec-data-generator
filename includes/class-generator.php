...
class Generator {
	...
	/**
	 * Post kinds that get a featured image. Tickets/attendees are excluded: a thumbnail on an
	 * RSVP ticket post is meaningless, and it used to double the (by far most expensive) part
	 * of a run.
	 */
	const IMAGE_POOL_SIZE = 8;

	const FEATURED_IMAGE_KINDS = [ 'event', 'page', 'post', 'venue', 'organizer', 'series' ];
	...
	public function add_attendees( int $ticket_id, int $quantity ): array {
		...
		$this->clear_attendee_cache( $event_id );
	...
	private function create_attendees_for_ticket( int $ticket_id, int $post_id ): array {
		...
		$this->clear_attendee_cache( $post_id );
	...
	/**
	 * Clears Event Tickets' per-post attendee cache (Tribe__Post_Transient, 1 hour TTL / 2 minutes
	 * on admin screens).
	 *
	 * Attendees here are written with raw wp_insert_post() — there's no ET API for this shape (see
	 * CLAUDE.md) — so nothing invalidates the cache ET itself populated earlier in the same run:
	 * `ticket_add()` warms it while the ticket still has zero attendees, and that empty list then
	 * sticks. The Tickets/Attendees screens report 0 attendees for a freshly generated post until
	 * it expires, which reads as "the generator didn't create any attendees".
	 */
	private function clear_attendee_cache( int $post_id ): void {
		if ( ! $post_id || ! function_exists( 'tribe' ) ) {
			return;
		}
		...
		try {
			...
			if ( method_exists( $rsvp, 'clear_attendees_cache' ) ) {
				$rsvp->clear_attendees_cache( $post_id );
			}
		} catch ( \Throwable $e ) {
			// A cache that can't be cleared is not worth failing a run over.
		}
	}
	...
	private function tag_generated( int $post_id, string $kind ): void {
		...
		if ( in_array( $kind, self::FEATURED_IMAGE_KINDS, true ) ) {
			$this->maybe_attach_featured_image( $post_id );
		}
	}

	/**
	 * Attaches one of this run's pooled placeholder images as the post's featured image, for any
	 * generated post type that actually declares thumbnail support.
	 */
	...
	/**
	 * Lazily creates a small pool of attachments (once per run — reusing any this run already
	 * created on an earlier chunk, same as the venue/organizer pools) and returns a random one.
	 *
	 * The pool exists because building an attachment per post is by far the most expensive thing
	 * in a run: a picsum HTTP round trip plus wp_generate_attachment_metadata()'s image resizing,
	 * ~0.6s per post — 70% of a run's wall clock, and the reason a 7k-event run crawled. Sharing
	 * IMAGE_POOL_SIZE images across the whole run makes it one meta write per post instead, while
	 * still giving QA visibly varied featured images.
	 */
	private function pick_image(): int {
		if ( empty( $this->image_pool ) ) {
			$this->image_pool = $this->find_run_posts( 'attachment' );
		}

		if ( empty( $this->image_pool ) ) {
			for ( $i = 0; $i < self::IMAGE_POOL_SIZE; $i++ ) {
				$id = $this->create_image( $i );
		...
		return $this->image_pool ? $this->image_pool[ array_rand( $this->image_pool ) ] : 0;
	}

	/**
	 * Creates one pooled attachment. Pool images have no post_parent (they're shared), so Cleanup
	 * deletes them by their own GENERATED_META_KEY tag rather than by cascade from a parent post.
	 */
	private function create_image( int $seed ): int {
		$bytes = $this->fetch_picsum_image( $seed + wp_rand( 1, 100000 ) );

		if ( ! $bytes ) {
			$bytes = Data::placeholder_image_bytes( $seed );
		}
		...
		$upload = wp_upload_bits( "loadgen-{$this->run_id}-{$seed}.png", null, $bytes );
	...
	/**
	 * Fetches a real placeholder photo from picsum.photos. Returns '' on any WP_Error/non-200/
	 * timeout so the caller falls back to the locally-generated placeholder — this must never
	 * block or fail a run just because picsum is unreachable.
	 */
